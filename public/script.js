...
}, { root: null, threshold: 0.55 }); // scroll-snap dimatikan, pakai viewport
...
function renderComment(doc) {
  ...
  // ===== Balas (muncul kalau admin sudah login)
  if (isAdmin) {
...
if (loginSubmit) {
  loginSubmit.addEventListener('click', function () {
    ...
    if (attempt === ADMIN_HASH_HEX) {
      ...
      setTimeout(function () { loginBackdrop.style.display = 'none'; location.reload(); }, 700); // reload biar form balas muncul
    } else {
